...
export async function exportData() {
  try {
    ...
    console.log(
      `Exported ${contacts.length} contacts, ${customFieldDefinitions.length} custom fields, ${groups.length} groups.`,
    )
...
export async function importData(data) {
  ...
  try {
    ...
    await db.transaction('rw', db.contacts, db.customFieldDefinitions, db.groups, async (tx) => {
      ...
      await Promise.all([db.contacts.clear(), db.customFieldDefinitions.clear(), db.groups.clear()])
      console.log('Existing data cleared.')

      console.log('Adding imported data...')

      const cleanedContacts = data.contacts.map((item) => {
        const { $$md, ...rest } = item
        return rest
      })
      const cleanedCustomFieldDefinitions = data.customFieldDefinitions.map((item) => {
        const { $$md, ...rest } = item
        return rest
      })
      const cleanedGroups = data.groups.map((item) => {
        const { $$md, ...rest } = item
        return rest
      })
      ...
      if (cleanedCustomFieldDefinitions.length > 0) {
        console.log(
          `Adding ${cleanedCustomFieldDefinitions.length} custom field definitions one by one to get new IDs...`,
        )
        ...
        for (const originalDef of cleanedCustomFieldDefinitions) {
          ...
          try {
            ...
            if (originalId !== undefined && newId !== undefined) {
              ...
              console.warn(
                `Could not map original custom field ID:`,
                originalDef,
                `Generated new ID:`,
                newId,
              )
            ...
            console.error(
              `Error adding custom field definition with original ID ${originalId}:`,
              originalDef,
              addError,
            )
        ...
        console.log(
          'Finished adding custom field definitions. Created mapping:',
          oldToNewCustomFieldIdMap,
        )
      } else {
          ...
          contact.customFields = contact.customFields
            .map((cf) => {
              ...
              if (oldToNewCustomFieldIdMap.has(cf.fieldId)) {
                ...
                console.warn(
                  `Contact ${contact.id} refers to custom field ID ${cf.fieldId}, but this ID was not found in the imported definitions map. This custom field will be skipped during import.`,
                )
      ...
      if (modifiedContacts.length > 0) {
        ...
        console.log(
          `Successfully imported ${modifiedContacts.length} contacts with updated custom field IDs.`,
        )
      } else {
