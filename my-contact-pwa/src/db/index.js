// src/db/index.js
// import Dexie from 'dexie'; // ایمپورت قبلی که مشکل داشت
import { Dexie } from 'dexie' // ایمپورت جدید به روش named export
...
db.version(2)
  .stores({
    ...
    contacts:
      '++id, name, lastName, phone, group, *additionalPhones.number, *addresses.city, *addresses.street, *customFields',
    groups: '++id, &name',
...
db.version(3)
  .stores({
    ...
    contacts:
      '++id, name, lastName, phone, group, createdAt, updatedAt, birthDate, *additionalPhones, *addresses, *customFields', // فیلد birthDate و createdAt/updatedAt اینجا اضافه شده
    groups: '++id, &name',
  ...
  .upgrade((transaction) => {
    ...
    console.log(
      'Database migrating from version 2 to 3. Added customFieldDefinitions store and new indexes/fields to contacts.',
    )
...
db.version(4)
  .stores({
    ...
    contacts:
      '++id, name, lastName, phone, group, createdAt, updatedAt, birthDate, *additionalPhones, *addresses, *customFields',
    groups: '++id, &name',
  ...
  .upgrade((transaction) => {
    ...
    console.log(
      "Database migrating from version 3 to 4. Adding 'settings' store and default record.",
    )
