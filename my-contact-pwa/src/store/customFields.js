  ...
  actions: {
    async loadFieldDefinitions() {
      ...
      try {
        ...
        console.log('تعاریف فیلدهای سفارشی با موفقیت از دیتابیس خوانده شدند:', this.fieldDefinitions);
      } catch (err) {
    ...
    async addFieldDefinition(fieldData) {
      ...
      const existingField = await db.customFieldDefinitions.where('label').equalsIgnoreCase(trimmedLabel).first();
      if (existingField) {
        ...
        try {
          const optionsToStore = fieldData.type === 'select'
          ? (fieldData.options ? toRaw(fieldData.options) : [])
              : [];
        ...
        console.log('  options:', optionsToStore, `(Type: ${typeof optionsToStore}, Is Array: ${Array.isArray(optionsToStore)})`);
