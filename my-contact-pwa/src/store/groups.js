  ...
  actions: {
    ...
    async deleteGroup(groupId) {
        ...
        try {
            ...
            const contactsInGroup = await db.contacts.where('group').equalsIgnoreCase(groupName).toArray(); // استفاده از ignoreCase
    ...
    async updateGroup(groupId, newName) {
        ...
        const existingGroupWithNewName = await db.groups.where('name').equalsIgnoreCase(trimmedNewName).first();
        ...
        try {
            ...
            console.log(`نام گروه با موفقیت از "${oldName}" به "${trimmedNewName}" تغییر یافت. ID: ${groupId}`);
            ...
            const contactsInOldGroup = await db.contacts.where('group').equalsIgnoreCase(oldName).toArray();
            console.log(`پیدا شد ${contactsInOldGroup.length} مخاطب در گروه قدیمی "${oldName}"`);
             ...
             if (contactsInOldGroup.length > 0) {
                  ...
                  await db.transaction('rw', db.contacts, async () => {
                      await db.contacts.bulkPut(updates);
                      console.log(`${updates.length} مخاطب مرتبط با گروه "${oldName}" به‌روزرسانی شدند به گروه "${trimmedNewName}".`);
