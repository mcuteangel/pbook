...
// تعریف یک Store جدید با اسم منحصر به فرد 'contactStore'
   ...
   getters: {
     ...
     filteredAndSortedContacts: (state) => {
       ...
       sorted.sort((a, b) => {
        ...
        if (state.sortField === 'createdAt' || state.sortField === 'updatedAt') {
            // اگر فیلد تاریخ بود، بر اساس timestamp مقایسه می‌کنیم
            const dateA = fieldA ? new Date(fieldA).getTime() : 0; // 0 برای تاریخ‌های خالی یا نامشخص
            ...
            comparison = String(fieldA).localeCompare(String(fieldB), 'fa', { sensitivity: 'base' }); // 'fa' برای فارسی، sensitivity: 'base' برای نادیده گرفتن تفاوت همزه و اعراب
        }
